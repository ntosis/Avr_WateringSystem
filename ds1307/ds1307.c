					...
					// ***************  seventh step for year
										...
										do {
										 ...
										 if (TurnDetected) {
											...
											if (up)
												clearwaterconsumption=0;
											else
												clearwaterconsumption=1;
											}
											TurnDetected = false;    // do NOT repeat IF loop until new rotation detected
...
void _draw(uint8_t idx)
{
	...
	if(idx==0) {
		...
		sprintf_P(buf,PSTR("Clear water consumption = %02d"),clearwaterconsumption);
